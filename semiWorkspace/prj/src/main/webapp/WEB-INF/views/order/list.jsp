...
<link rel="stylesheet"
	href="/app/resources/css/common/nav-with-header.css">
			...
			<section class="profile__header--container">
				...
				<div class="profile__statuses">
					<c:forEach var="status" items="${statusDtoList}">
						<a href="/app/orders/list?statusNo=${status.statusNo}"
							class="status__link">
							<div class="profile__status status-${status.statusNo}"
								data-status="${status.statusName}">
								<span class="profile__status--name">${status.statusName}</span>
								<span class="profile__status--count">${status.count}건</span>
							</div>
						</a>
					</c:forEach>
				</div>
						...
						<tbody>
							<c:forEach var="dto" items="${requestScope.dtoList}">
								<tr>
									<td>
										<div class="product-details">
											<a href="/app/products/${dto.productNo}" class="product-link">
												...
												src="/app/resources/img/product/${dto.thumbnailFilename}"
												alt="Product image">
											</a>
											<div class="product-detail__text">

												<p class="orders__seller">
													<a href="/app/search-by-store?sellerNo=${dto.sellerNo}">[${dto.sellerName}]</a>
												</p>
												<p class="orders__product">
													<a href="/app/products/${dto.productNo}"><strong>${dto.productName}</strong></a>
												</p>
									...
									<td>${dto.ordersNo}</td>
									<td class="orders__price-quantity">${dto.productPrice}원<br>
									<span class="orders-quantity">${dto.orderDetailQuantity}개</span></td>
									<td class="orders__total-price">${dto.orderDetailPrice}원</td>
									<td class="orders__status">${dto.orderDetailStatusName}</td>
									<td class="action-td">
										<button class="ask-btn" data-orders-no="${dto.ordersNo}"
											data-seller-no="${dto.sellerNo}"
											data-order-detail-no="${dto.orderDetailNo}"
											data-member-no="${sessionScope.loginMemberVo.memberNo}">주문
											문의</button> <br>
										<button class="review-btn" data-product-no="${dto.productNo}"
											data-orders-no="${dto.ordersNo}"
											data-member-no="${sessionScope.loginMemberVo.memberNo}"
											data-order-detail-no="${dto.orderDetailNo}">리뷰 작성</button>
									</td>
								</tr>
							</c:forEach>
						</tbody>
